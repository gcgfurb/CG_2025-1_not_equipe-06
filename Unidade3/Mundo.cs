...
namespace gcgcg
{
    public class Mundo : GameWindow
    {
        ...
        private Objeto objectInProgress;
      ...
      Console.WriteLine("Tamanho interno da janela de desenho: " + ClientSize.X + "x" + ClientSize.Y);
        ...
        protected override void OnUpdateFrame(FrameEventArgs e)
        {
            ...
            if (estadoTeclado.IsKeyPressed(Keys.Space))
            {
                ...
                objetoSelecionado = mundo.GraphSceneSearchNext(objetoSelecionado);
            ...
            if (estadoTeclado.IsKeyPressed(Keys.Enter))
            {
                ...
                objetoSelecionado = objectInProgress;
            ...
            if (estadoTeclado.IsKeyDown(Keys.V) && objetoSelecionado != null)
            {
                ...
                //mouse position
                var mousePosition = new Ponto4D(MousePosition.X, MousePosition.Y);
                //point independent of the sceen coordinates
                var sruPoint = Utilitario.NDC_TelaSRU(Size.X, Size.Y, mousePosition);
                var position = objetoSelecionado.PontoMaisPerto(sruPoint, false);

                if(position > -1)
                    objetoSelecionado.PontosAlterar(sruPoint, position);
            }
            ...
            if (estadoTeclado.IsKeyPressed(Keys.E) && objetoSelecionado != null)
            {
                ...
                var mousePosition = new Ponto4D(MousePosition.X, MousePosition.Y);
                var sruPoint = Utilitario.NDC_TelaSRU(Size.X, Size.Y, mousePosition);
                var position = objetoSelecionado.PontoMaisPerto(sruPoint, true);
            }
            ...
            if (estadoTeclado.IsKeyPressed(Keys.P) && objetoSelecionado != null)
            {
                ...
                if (objetoSelecionado is Poligono poly)
                    poly.DrawnPolygonMode();
            }
            ...
            if (MouseState.IsButtonPressed(MouseButton.Right))
            {
                ...
                {
                    //min 2 points
                    var poligononPoints = new List<Ponto4D>
                    {
                        sruPoint,
                        sruPoint
                    };

                    //verify if the selected point is being created or edited
                    if (objetoSelecionado != null)
                    {
                        objectInProgress = new Poligono(objetoSelecionado, ref rotuloAtual, poligononPoints);
                        objetoSelecionado = null;
                    }

                    else
                        objectInProgress = new Poligono(mundo, ref rotuloAtual, poligononPoints);
                }

                else
                    objectInProgress.PontosAdicionar(sruPoint);
            }
            ...
            if (MouseState.IsButtonPressed(MouseButton.Left) && objetoNovo == null)
            {
                ...
                //}
                foreach(Objeto obj in mundo.ObjectList())
                {
                    var box = obj.Bbox().Dentro(sruPonto);
